...
def hgs_secant(fun, n0, opt_sec, ilevel=0):
    ...
    * ESEIAAT UPC
    ...
    if x2 - x1 > 1500:  # Try to fit to a parabola and solve the eq.
        x3 = (x2 + x1) / 2
        y3, _ = fun(x3, n0)
        a = (y1 - (y2 - y3) / (x2 - x3) * x1 - y3 + x3 * (y2 - y3) / (x2 - x3)) / (
            x1**2 + (x1 - x3) * (x3**2 - x2**2) / (x2 - x3) - x3**2
        )
    ...
    for ii in range(maxiter):
        ...
        if info:
            print(
                f"ii={ii} x1={x1:.2E} y1={y1:.2E} xc={xc:.2E} yc={yc:.2E} x2={x2:.2E} y2={y2:.2E} tolx={x2-x1:.2E}"
                f" toly={yc:.2E}\n"
            )
        ...
        if yc * y1 > 0:  # Change limits
            if x2 - xc < fchange or xc - x1 < fchange:
                Bisec = True
            y1 = yc
            x1 = xc
            n1 = n
