...
def hgs_Tp(
    ...
    * ESEIAAT UPC
    """
    if typ not in ["H", "T"]:
        raiseError(f"Wrong type = {type}")
    if type(V0) in (float, int, np.float64, np.float32):
        V0 = [V0] * len(species)
    if len(V0) == 1:
        V0 = [V0[0]] * len(species)
    ...
    Tp, n, flag = hgs_Tp_ids(
        ids, n0, typ, V0, P, flow, solver, Tstar, opt_eq, opt_sci, opt_sec, hgs_data
    )
