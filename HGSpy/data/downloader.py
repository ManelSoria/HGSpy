...
def parse_chemkin_elem(e):
    ...
    n = e[:i1].strip()
    if n.lower() == "air":
        return None  # Skip air as no composition is given
    if "mgcl2" in n.lower():
        return None  # Skip as it is horrible to parse
    out["name"] = n
    e = e[18 + 6 :]
...
def download(download=True, info=False):
    ...
    *+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*+*
    ...
    if not os.path.isfile(RAWDATA):
        raiseError("Database not expected in the right directory <%s>!" % RAWDATA)
    ...
    if info:
        print("Molar Mass calculation ...", end=" ")
    for ii in range(len(hgs_new)):
        mm = 0.0
        for n, e in zip(hgs_new["nat"][ii], hgs_new["ena"][ii]):
            if e in Mendeley.keys():
                mm += n * Mendeley[e]
