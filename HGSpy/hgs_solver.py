...
def hgs_solver(
    ...
    * ESEIAAT UPC
    ...
    if solver == "hgs_secant":
        Tp, n, flag = hgs_secant(
            lambda Ti, ni: hastobezero(Ti, P, ni, ids, V0, opt_eq, hgs_data), n0, opt_sec
        )
    else:
        # Use a solver from the scipy.optimize package
        Tp = getattr(scipy.optimize, solver)(
            lambda Ti: hastobezero(Ti[0], P, n0, ids, V0, opt_eq, hgs_data)[0], Tstar, **opt_sci
        )
