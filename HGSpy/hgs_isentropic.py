...
def hastobeM_shifting(V1, m, h1, Pstar, S, ids, n, opt_eq, hgs_data):
    ...
    """
    Tstar, n, flag = hgs_secant(
        lambda Ti, ni: hastobeS_shifting(S, Ti, Pstar, ids, ni, opt_eq, hgs_data),
        n,
        opt_sec,
        ilevel=1,
    )
    if flag != 1:
        raiseError("uhhh error in hastobeM HGSsecant failed flag=%d" % flag)
...
def hastobeM_frozen(V1, m, h1, Pstar, S, ids, n, opt_eq, hgs_data):
    ...
    """
    Tstar, n, flag = hgs_secant(
        lambda Ti, ni: hastobeS_frozen(S, Ti, Pstar, ids, ni, opt_eq, hgs_data),
        n,
        opt_sec,
        ilevel=1,
    )
...
def hgs_isentropic_ids(
    ids, n0, T0, P0, type, V1, flow, solver, Tstar, opt_eq, opt_sci, opt_sec, hgs_data
):
    """
    ...
    if type == "P":
        ...
        Mm2, a2, H2 = hgs_prop_ids(
            ids, n, [Tp] * len(ids), P1, ["Mm", "a", "H"], hgs_data
        )  # Outlet properties
        ...
        P1, n, flag = hgs_secant(
            lambda Pi, ni: hastobeM(V1, m1, h1, Pi, S, ids, ni, opt_eq, hgs_data), n0, opt_sec2
        )
        # Flag error return
        if flag != 1:
...
def hgs_isentropic(
    ...
    * ESEIAAT UPC
    """
    if typ not in ["P", "M"]:
        raiseError(f"Wrong type = {typ}")
    if type(T0) in (float, int, np.float64, np.float32):
        T0 = [T0] * len(species)
    if len(T0) == 1:
        T0 = [T0[0]] * len(species)
    ...
    Tp, n, v2, V2, flag = hgs_isentropic_ids(
        ids, n0, T0, P0, typ, V1, flow, solver, Tstar, opt_eq, opt_sci, opt_sec, hgs_data
    )
