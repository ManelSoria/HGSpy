...
def sound(gamma, rg, T):
    ...
    * ESEIAAT UPC
    """
    return np.sqrt(gamma * T * rg * 1.0e3)
...
def hgs_single(species, prop, T, P, hgs_data=HGSData.load()):
    ...
    * ESEIAAT UPC
    ...
    if type(T) in (float, int, np.float64, np.float32):
        T = [T]
...
def hgs_prop(species, n, T, P, *args, hgs_data=HGSData.load()):
    ...
    * ESEIAAT UPC
    ...
    if type(species) is str:
        species = [species]
    if type(n) in (float, int, np.float64, np.float32):
        n = [n]
    if type(T) in (float, int, np.float64, np.float32):
        T = [T] * len(species)
    if len(T) == 1:
    ...
    if len(n) is not len(species):
        raiseError(
            "Ups..., Species (%d) and mols (%d) lengths are not the same!" % (len(species), len(n))
        )
