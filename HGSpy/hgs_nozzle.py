...
def hgs_nozzle(
    ...
    * ESEIAAT UPC
    ...
    for ii in range(len(P)):
        ...
        T[ii], n[:, ii], _, M[ii], flag = hgs_isentropic_ids(
            ids, n0, T0, P0, "P", P[ii], flow, solver, Tstar, opt_eq, opt_sci, opt_sec, hgs_data
        )
        if not flag == 1:
            raiseWarning("HGSnozzle failed to converge/1 flag=%d", flag)
        Rg, a, Mm = hgs_prop_ids(
            ids, n[:, ii], [T[ii]] * len(ids), P[ii], ["Rg", "a", "Mm"], hgs_data
        )  # kJ/(kg*K), m/s, g/mol
